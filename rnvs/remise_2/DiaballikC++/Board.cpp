...
#include "Players.h"    // rnvs : ajout
using namespace PlayersSpace; // rnvs : ajout
...
Board::Board(int width, int height):
    width_(width),
    height_(height),
    board_ (vector<vector<Piece>>(width_, vector<Piece>(height_,
                                  Piece(Color::NO))))
{}
...
void Board::initBoard()
{
    ...
    for (int i = 0; i < height_; i++)   // rnvs : plantage si plateau pas carré : i : 0 -> height or ci-dessus j va de 0 à width : ko
    {
        board_[0][i] = Piece(Color::BLACK);
        board_[6][i] = Piece(Color::WHITE);
    }

    board_[0][3].setBool(); // rnvs : balle pas placée au milieu : ko
    board_[6][3].setBool(); // rnvs : balle pas placée au milieu : ko et pas sur dernière ligne
}
...
ostream & Board::showBoard (ostream & c )
{
    for (int i = 0; i < width_; i++)
    {
        ...
        for (int j = 0; j < height_; j++)
        {
            ...
            {
                c << " " << static_cast<std::underlying_type<Color>::type>
                  (board_[i][j].getColor()) << "b";
...
void Board::move(int ox, int oy, int dx, int dy, Color currentColor)
{
    if (checkMove(ox, oy, dx, dy))
    {
        if (board_[dx][dy].getColor() == Color::NO &&
                board_[ox][oy].getColor() == currentColor &&
                !board_[ox][oy].isInside())
        {
...
{
    bool ok = false;
    if (ox >= 0 & ox<width_ & oy >= 0 & oy <height_ & dx >= 0 & dx
            <width_ & dy >= 0 & dy < height_)
    {
        if ((dx == ox - 1 & dy == oy) || (dx == ox & dy == oy + 1) ||
                (dx == ox + 1 & dy == oy) || (dx == ox & dy == oy - 1))
        {
            ok = true;
        }
        else
...
void Board::passe(int dx, int dy, Players currentPlayer)
{
    ...
    for (int i = 0; i < width_; i++)
    {
        for (int j = 0; j < height_; j++)
        {
            if (currentPlayer.getColor() == board_[i][j].getColor() &&
                    board_[i][j].isInside())
            {
                ox = i;
                oy = j;
    ...
    {
        if (board_[dx][dy].getColor() == board_[ox][oy].getColor() &&
                !isInsideBall(dx, dy))
        {
...
Piece Board::direction(int ox, int oy, int width, int height,
                       Color color)
{
    int directionWidth = ox + width;
    int directionHeight = oy + height;

    while (directionWidth >= 0 & directionWidth
            < width_ & directionHeight >= 0 & directionHeight < height_
